...
public class SculkEyeItem extends Item {
    public SculkEyeItem(Item.Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (hitResult.getType() == HitResult.Type.BLOCK && world.getBlockState(((BlockHitResult) hitResult).getBlockPos()).isOf(Blocks.END_PORTAL_FRAME)) {
            ...
            if (world instanceof ServerWorld) {
                ...
                if (blockPos != null) {
                    SculkEyeEntity sculkEyeEntity = new SculkEyeEntity(world, user.getX(), user.getBodyY(0.5D), user.getZ());
                    sculkEyeEntity.setItem(itemStack);
                    sculkEyeEntity.initTargetPos(blockPos);
                    world.spawnEntity(sculkEyeEntity);
                    ...
                    world.playSound(null, user.getX(), user.getY(), user.getZ(), SoundEvents.BLOCK_SCULK_SENSOR_CLICKING, SoundCategory.NEUTRAL, 0.5F, 0.4F / (world.getRandom().nextFloat() * 0.4F + 0.8F));
