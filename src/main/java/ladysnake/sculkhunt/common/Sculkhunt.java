...
public class Sculkhunt implements ModInitializer {
    ...
    public static final int SPAWN_RADIUS = 70;
    ...
    public void onInitialize() {
        ...
        ServerPlayerEvents.AFTER_RESPAWN.register((oldPlayer, newPlayer, alive) -> {
            ...
            if (SculkhuntComponents.SCULK.get(newPlayer).isSculk()) {
                newPlayer.getAttributes().getCustomInstance(EntityAttributes.GENERIC_MAX_HEALTH).setBaseValue(12f);
                newPlayer.getAttributes().getCustomInstance(EntityAttributes.GENERIC_MOVEMENT_SPEED).setBaseValue(0.11f);
                newPlayer.setHealth(newPlayer.getMaxHealth());
                newPlayer.getAttributes().getCustomInstance(EntityAttributes.GENERIC_ATTACK_DAMAGE).setBaseValue(2f);
                ...
                if (!players.isEmpty()) {
                    ...
                    if (!catalysts.isEmpty()) {
                        catalysts.sort((o1, o2) -> prey.getPos().distanceTo(o1.getPos()) > prey.getPos().distanceTo(o2.getPos()) ? 1 : 0);
            ...
            server.getWorlds().forEach(world -> {
                if (world.getGameRules().get(SculkhuntGamerules.SCULK_CATALYST_SPAWNING).get() && world.random.nextInt(world.getGameRules().get(SculkhuntGamerules.SCULK_CATALYST_SPAWNING_DELAY).get()) == 0) {
                    for (ServerPlayerEntity player : world.getPlayers()) {
                        if (world.getEntitiesByClass(SculkCatalystEntity.class, player.getBoundingBox().expand(100), sculkCatalystEntity -> true).isEmpty()) {
