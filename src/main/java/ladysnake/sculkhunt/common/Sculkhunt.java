            ...
            if (SculkhuntComponents.SCULK.get(newPlayer).isSculk()) {
                ...
                if (!players.isEmpty()) {
                    ...
                    if (!catalysts.isEmpty()) {
                        // filter catalysts that aren't in a 15 block radius of the player
                        catalysts = catalysts.stream().filter(sculkCatalystEntity -> Math.sqrt(sculkCatalystEntity.getBlockPos().getSquaredDistance(prey.getBlockPos())) >= 15).collect(Collectors.toList());
                        if (!catalysts.isEmpty()) {
